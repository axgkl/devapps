...
def test(ctx, match=os.environ.get('match', '')):
    ...
    cmd = 'pytest -p no:randomly -c config/pytest.ini -k "%s" tests'
    title = 'Running tests with $pytest_autodoc set'
    ctx.run(cmd % match, title=title, pty=PTY, capture=False)
...
def badges(ctx):
    ...
    u = (
        'hg+https://axchange.github.com/scm/hg/noauth/badges::/tmp/badges_%(USER)s'
        % os.environ
    )
    ctx.run(
...
def release(ctx, version=os.environ.get('version')):
    ...
    # poetry config repositories.ax https://artifacts-internal.github.com/artifactory/api/pypi/pypi-ax-sources
    # (goes to ~/.config/pypoetry/config.toml)
    print('all repos')
    print('https://artifacts-internal.github.com/artifactory/pypi-ax-sources/')
    print('Hint: A conflict error is normal - the file is uploaded then.')
    user = user or input('artifacts-internal username > ')
